...
if len(sys.argv) == 1:
    print("SNIPH version 1.0\n")

    inp = raw_input("Input # of dimensions, N, for cipher (empty for default): ")
    if len(inp) == 0:
        N = 4
    else:
        N = int(inp)
    ...
    R = C = 0
    dim = raw_input("Enter dimensions for cipher table, in range 3-10, in form # rows x #cols (empty for default): ")
    if len(dim) == 0:
        # default
        R = C = 3
    else:    
    ...
    phrase = raw_input("Enter passphrase: ")
    if len(phrase) == 0:
